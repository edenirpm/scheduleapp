...
type
  ...
  public
    ...
    function Agendamento:TJSonObject;
    function doLogin(Auser,Apassword:string):TJsonValue;
    function Confirm(NomeEmpresa:string;indexfuncionario, indexMarcacoes:integer):boolean;
    function Desmarcar(NomeEmpresa:string;indexfuncionario,indexmarcacoes:integer):boolean;
     function Remarcar(NomeEmpresa:string;indexfuncionario,indexmarcacoes:integer):boolean;
...
function Tsm.Agendamento: TJsonObject;
var
oCrs:IMongoCursor;
Agendados:TAgendados;
...
function Tsm.Confirm(NomeEmpresa:string;indexfuncionario, indexMarcacoes:integer): boolean;
begin
   try
       try
        FDConnection1.Connected:=true;
        Conexao:=TMongoConnection(FDConnection1.CliObj);
        Conexao['AgendamentoOnLine']['Documentos'].Update()
        .Match()
          .add('empresa.nome',NomeEmpresa)
            .&End
         .Modify()
           .&set
             .Field('funcionarios.items.'+inttostr(indexFuncionario)+'.agendamentos.marcacoes.items.'+inttostr(indexMarcacoes)+'.status','Confirmado')
           .&End
          .&End
          .Exec;
        finally
...
function Tsm.Desmarcar(NomeEmpresa: string; indexfuncionario,
  indexmarcacoes: integer): boolean;
begin
   try
       try
        FDConnection1.Connected:=true;
        ...
        .Match()
         ...
         .Modify()
           .&set
             .Field('funcionarios.items.'+inttostr(indexFuncionario)+'.agendamentos.marcacoes.items.'+inttostr(indexMarcacoes)+'.status','Cancelado')
           .&End
          .&End
          .Exec;
        finally
...
function Tsm.Remarcar(NomeEmpresa: string; indexfuncionario,
  indexmarcacoes: integer): boolean;
begin
   try
       try
        ...
        .Match()
         ...
         .Modify()
           .&set
             .Field('funcionarios.items.'+inttostr(indexFuncionario)+'.agendamentos.marcacoes.items.'+inttostr(indexMarcacoes)+'.status','Remarcar')
           .&End
          .&End
          .Exec;
        finally
